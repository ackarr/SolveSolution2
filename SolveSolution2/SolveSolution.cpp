...
int    ChooseMode       (int argc, char* argv[], ModesParam* modes_list);
...
int RunSelectTest (ModesParam* modes_list)
{
    for(int i = 0; i < QUANT_MODES; i++)
    {
        if(modes_list[i].switcher)
        {
            if(!strcmp(modes_list[i].mode_name, "test"))
            {
                BadCode(RunAllHand());
                BadCode(RunAllRandom());
            }

            if(!strcmp(modes_list[i].mode_name, "test_h"))
            {
                BadCode(RunAllHand());
            }

            if(!strcmp(modes_list[i].mode_name, "test_r"))
            {
                BadCode(RunAllRandom());
            }
        }
